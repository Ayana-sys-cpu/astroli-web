...
-- vote_counts view depends on votes.journey_id (discovered when this
-- migration first failed with P3018 / 2BP01) — rebuild it on class_id
-- before dropping the column. Neither consumer (app/api/winner/route.ts,
-- app/api/vote-counts/route.ts) selects or filters on journey_id, only
-- vote_session_id and big_idea_id, so this is a safe drop-in replacement.
-- -----------------------------------------------------------------------------
...
CREATE VIEW vote_counts AS
  SELECT
    ...
    COUNT(*) AS vote_count
  FROM votes
  GROUP BY vote_session_id, class_id, big_idea_id;
