/**
 * Targeted patch: sets opening_quick_replies on all existing missions and planets.
...
// ── Planets (matched by title) ──────────────────────────────────────────────────
const planetPatches: { id: string; label: string; qr: { label: string; value: string }[] }[] = [
  {
    id: '5703d3de-2c22-43c5-8976-dd923844e135', // The Central Role of the Catholic Church in Medieval Society
    label: 'Planet 1-1 (Church)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 1-2 (Pope vs Emperor)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 2-2 (Feudal Hierarchy)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 2-5 (Rise of Cities)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 3-1 (Judaism in Christian Worldview)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 3-2 (Foundations of Islam)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 3-3 (Spread of Islam / Jihad)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 3-4 (Dhimmi System)',
    qr: [
  ...
  },
  {
    ...
    label: 'Planet 3-7 (Crusader Kingdom)',
    qr: [
...
async function main() {
  ...
  console.log('Patching planets...')
  for (const { id, label, qr } of planetPatches) {
    const result = await prisma.planet.updateMany({ where: { id }, data: { openingQuickReplies: qr } })
    console.log(`  ${result.count === 1 ? '✓' : '✗'} ${label}`)
    planetCount += result.count
  }

  console.log(`\nDone — ${missionCount}/3 missions, ${planetCount}/16 planets patched.`)
}
