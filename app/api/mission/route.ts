...
export async function GET(req: NextRequest) {
  ...
  // ── Only bot conversation fields are translated — curriculum stays English ──
  // Translated: openingMessage, openingMessage2, qaAnswers, missionQaAnswers
  // English:    question, worldBrief, missionBrief, projectTitle, projectObjective, chapter
  const botTx: Record<string, any> = missionLanguage === 'he'
    ? ((mission as any).translations as Record<string, any>)?.he ?? {}
    : {};

  // ── Derive computed fields (always English) ───────────────────────────────
  const q = mission.question as string;
  const missionBrief = (mission.mission_brief as string | null)
    ?? (q.length > 60 ? q.slice(0, 57) + '…' : q);

  const projectDescRaw = mission.project_description as string;
  ...
  const pipMission: OrinMission = {
    ...
    projectObjective,
    openingMessage:    (botTx.opening_message ?? mission.opening_message) as string,
    openingMessage2:   (botTx.opening_message_2 ?? mission.opening_message_2 as string | null) ?? '',
    missionBrief,
    ...
    qaAnswers:        (botTx.qa_answers ?? mission.qa_answers as string[] | null) ?? [],
    missionQaAnswers: (botTx.mission_qa_answers ?? mission.mission_qa_answers as string[] | null) ?? [],
  };
