...
export interface JourneyCardData {
  ...
  activeMissionQuestion: string | null;
...
export default function JourneyCard({ journey }: { journey: JourneyCardData }) {
  ...
  const taglineVisible = journey.status === 'live' && !!journey.activeMissionQuestion;
  const cardOpacity    = journey.status === 'done' ? 0.75 : 1;

  const isVotingUrgent = journey.status === 'voting' &&
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: '16px 18px 18px', display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          {/* Tagline — active mission question on LIVE; space reserved on other states */}
          <div style={{
            fontSize: 11,
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <span style={{
              fontSize: isVotingUrgent ? 11 : 9,
              fontWeight: isVotingUrgent ? 700 : 400,
              letterSpacing: '0.07em',
              fontFamily: 'var(--font-inter)',
              color: STATUS_NOTE_COLOR[journey.status],
              animation: isVotingUrgent ? 'pulse-voting 1.5s ease-in-out infinite' : 'none',
            }}>
              {isVotingUrgent ? `⏰ ${journey.statusNote}` : journey.statusNote}
            </span>
          </div>
          ...
          <div style={{
            ...
            {journey.attentionCount > 0 ? (
              <span style={{ fontSize: 9, color: '#B45309', letterSpacing: '0.07em', fontWeight: 600 }}>
