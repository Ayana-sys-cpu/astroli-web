...
import ParentSignalBanner from './ParentSignalBanner';
import PerformanceBadge from '@/components/teacher/drill-down/PerformanceBadge';
...
export default function ParentThisWeekView({ data, selectedJourneyId, onSwitchToAllTime }: Props) {
      ...
      <div style={{
        ...
        {activeMission ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '0 4px', marginBottom: 14 }}>
            <span style={{ fontSize: 11, color: 'rgba(26,26,46,0.45)', fontWeight: 500 }}>Active mission</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#1a1a2e', lineHeight: 1.4 }}>{activeMission.title}</span>
          </div>
        ) : (
          <div style={{ padding: '0 4px', marginBottom: 14 }}>
            <span style={{ fontSize: 12, color: 'rgba(26,26,46,0.35)', fontStyle: 'italic' }}>No active mission</span>
          </div>
        )}

        {peakPerformance && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '0 4px', marginBottom: 8 }}>
            <span style={{ fontSize: 11, color: 'rgba(26,26,46,0.45)', fontWeight: 500 }}>Highest level reached</span>
        ...
        {activeSubjects.length > 0 && (
          <div style={{
            ...
            <span style={{ fontSize: 11, color: 'rgba(26,26,46,0.45)', fontWeight: 500 }}>Subjects this mission</span>
            <span style={{ fontSize: 20, fontWeight: 800, color: '#1a1a2e', lineHeight: 1 }}>{activeSubjects.length}</span>
          </div>
        )}
      </div>
        ...
        {activeSubjects.length === 0 ? (
          <div style={{ padding: '32px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            <p style={{ fontSize: 13, color: 'rgba(26,26,46,0.4)', margin: 0 }}>
              {activeMissionId ? 'No subjects in the active mission yet.' : 'No active mission in this journey right now.'}
            </p>
            <button
              className="dd-btn"
          ...
          <div>
            <div style={{ padding: '12px 20px 4px' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(26,26,46,0.5)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            ...
            {activeSubjects.map((subject) => (
              <SubjectRow
                ...
                onToggle={() => setExpandedPlanetId(expandedPlanetId === subject.planetId ? null : subject.planetId)}
              />
            ))}
          </div>
        )}
